...
class SpecialTaskFragment : Fragment(R.layout.fragment_special_task) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val prefs = requireContext().getSharedPreferences("downloads_special", Context.MODE_PRIVATE)
    ...
    private fun sharePost(post: InstaPost) {
        ...
        val files: List<java.io.File> = if (!post.isVideo && post.isCarousel) {
            ...
            listOf(
                if (!post.localPath.isNullOrBlank()) java.io.File(post.localPath!!) else java.io.File(targetDir, fileName)
            )
        }
